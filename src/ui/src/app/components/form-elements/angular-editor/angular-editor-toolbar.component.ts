...
export class AngularEditorToolbarComponent {
  ...
  table: TableSelectOption = { row: -1, col: -1 };
   ...
   * insert URL link
   */
  insertTable(tableSize: TableSelectOption) {
    this.execute.emit('');
    console.log('insertTable', tableSize);
    this.editorService.insertTable(tableSize);
  }

  insertPageBreak() {
    this.execute.emit('');
    const html = '<br><span style="display:inline-block;width:100%;height:10px;border:2px dotted #EEE;background:#f9f9f9;margin:10px 0;pointer-events: none;"><span class="page-break" style="page-break-after: always;"></span></span><br>';
    this.editorService.insertHtml(html);
  }

  insertTag(event: string) {
    console.log('insertTag', event)
    this.execute.emit('');
    this.editorService.insertHtml(event);
  }
